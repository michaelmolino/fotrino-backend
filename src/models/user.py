...
class User(UserMixin):
    def __init__(self, id, identity_provider, external_id, name, email, profile_pic, country):
        self.id = int(id)
    ...
    def get(id):
        ...
        user = User(
            id=id, identity_provider=row[0], external_id=row[1], name=row[2], email=row[3], profile_pic=row[4], country=row[5]
        )
        return user
    ...
    def get_user_from_email(email):
        ...
        if not row:
            return None
        user = User(
            id=row[0], identity_provider=row[1], external_id=row[2], name=row[3], email=email, profile_pic=row[4], country=row[5]
        )
        return user
